...
export default function ParamControls({ params, onChange }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-5">
      <Section title="LLTV">
        <ParamSlider label="L_YX" value={params.lYX} min={0} max={1} step={0.01} onChange={set("lYX")} />
        <ParamSlider label="L_XY" value={params.lXY} min={0} max={1} step={0.01} onChange={set("lXY")} />
      </Section>

      <Section title="Price">
        <ParamSlider label="p_x" value={params.px} min={0.01} max={10} step={0.01} onChange={set("px")} />
      ...
      <Section title="Concentration">
        <ParamSlider label="c_x" value={params.cx} min={0} max={0.99} step={0.01} onChange={set("cx")} />
        <ParamSlider label="c_y" value={params.cy} min={0} max={0.99} step={0.01} onChange={set("cy")} />
      </Section>
