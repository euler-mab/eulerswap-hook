...
/// @notice Corrected booster math for two-sided equity.
///
...
///     b = (H*E0$ + E1$*L1) / (H*E0$*beta - E1$*L1*alpha)
///     alpha = sqrt(1.05) - 1 = 0.02470
///     beta  = 1 - 1/sqrt(1.05) = 0.02410
///     b = (1.01*452 + 440*0.85) / (1.01*452*0.02410 - 440*0.85*0.02470)
///       = 830.5 / 1.77 = 470
///
///   New eq0 = 470 * 452 = 212,440 USDC
///   New eq1 = 470 * 0.2126 = 99.92 WETH
///   Depth per side: ~$5,120 USDC / ~$4,985 WETH
///   Health at X-boundary: ~1.010
contract ReconfigureUsdcWeth is Script {
    ...
    // 470x boost from ~$892 two-sided equity, +/-5% range
    uint112 constant EQ0 = 212440000000;                      // 212,440 USDC (6 decimals)
    uint112 constant EQ1 = 99922000000000000000;               // 99.922 WETH (18 decimals)
