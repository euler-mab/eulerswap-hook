...
/// @notice Corrected booster math: X = E * LTV / (H * beta - LTV * alpha)
///   E=500, LTV=0.94, H=1.01, r=1%, c=0
///   alpha = sqrt(1.01) - 1 = 0.004988
///   beta  = 1 - 1/sqrt(1.01) = 0.004963
///   X = 470 / (1.01*0.004963 - 0.94*0.004988) = 470 / 0.000324 = 1,450,000
///
///   Health at Y-boundary: (500 + 7232) * 0.94 / 7196 = 1.010
///   Health at X-boundary: 7232 * 0.94 / (7196 - 500) = 1.015
///   Depth per side: ~$7,200
contract ReconfigureUsdcUsdt is Script {
    ...
    // Corrected equilibrium: ~2900x boost from 500 USDC equity
    uint112 constant EQ = 1450000000000; // 1,450,000 (6 decimals)
