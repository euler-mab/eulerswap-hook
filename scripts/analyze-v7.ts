/**
 * V7 hook performance analysis — swaps since V7 deployment only.
 * Uses per-block Uniswap prices for exact valuation.
 *
 * Usage: npx tsx scripts/analyze-v7.ts
 */
...
const V7_DEPLOY = 24651832n;
const DEC0 = 6, DEC1 = 18;
...
async function main() {
  ...
  console.log("V7 deployed at block " + V7_DEPLOY + ", current block " + currentBlock);
  console.log("Scanning " + Number(currentBlock - V7_DEPLOY) + " blocks...\n");

  // Fetch V7-era swaps
  const swaps: SwapData[] = [];
  let cursor = V7_DEPLOY;
  ...
  console.log("Found " + swaps.length + " swaps since V7 deploy\n");
  ...
  const days = Number(currentBlock - V7_DEPLOY) * 12 / 86400;
  ...
  console.log("  V7 Hook Performance (per-block Uniswap pricing)");
  console.log("=".repeat(60));
  ...
  // V4 comparison: swaps before V7
  console.log("\n" + "=".repeat(60));
  console.log("  For comparison — V4 metrics (same pool, pre-V7):");
